...
func maybeUpgradeForeignKeyRepOnIndex(
	...
	for refIdx := range idx.ReferencedBy {
		...
		originIndex, err := otherUnupgradedTables[ref.Table].FindIndexByID(ref.Index)
		if err != nil {
			return false, err
		}
...
func (desc *TableDescriptor) maybeDowngradeForeignKeyRepresentation(
	...
	// No need to process mutations, since only descriptors written on a 19.2
	// cluster (after finalizing the upgrade) have foreign key mutations.
	for i := range descCopy.OutboundFKs {
		fk := &descCopy.OutboundFKs[i]
		if _, ok := otherUnupgradedTables[fk.ReferencedTableID]; !ok {
			tbl, err := getTableDescFromIDRaw(ctx, protoGetter, fk.ReferencedTableID)
			if err != nil {
				return false, nil, err
			}
			otherUnupgradedTables[fk.ReferencedTableID] = tbl
		}
