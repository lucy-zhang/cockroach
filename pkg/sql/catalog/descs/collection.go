...
func (tc *Collection) GetDatabaseByName(
	...
	getDatabaseFromStore := func() (desc catalog.Descriptor, found bool, err error) {
		return getDescriptorFromStore(
...
func (tc *Collection) getUncommittedDescriptorNew(
	...
	for i := len(tc.uncommittedDescriptors) - 1; i >= 0; i-- {
		...
		// is seeing the effect of it.
		//
		// Only return at this point if the desired name doesn't match the
		// descriptor name.
		for _, drain := range mutDesc.GetDrainingNames() {
			if drain.Name == name &&
				drain.ParentID == dbID &&
				drain.ParentSchemaID == schemaID &&
				mutDesc.GetName() != name {
