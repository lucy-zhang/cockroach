...
func (sc *SchemaChanger) AddConstraints(
	...
	// Publish backreferences for foreign keys
	...
	for _, c := range constraints {
		...
		// TODO (lucy): figure out what to do when the backref index (or table?) is dropped before this can run
		_, err := sc.leaseMgr.Publish(ctx, c.ForeignKey.Table,
			func(desc *sqlbase.MutableTableDescriptor) error {
				...
				found := false
				for _, br := range idx.ReferencedBy {
					if br.Table == backref.Table && br.Index == backref.Index {
						log.VEventf(
							ctx, 2,
							"backfiller tried to add backreference %+v to table %d but it already existed, presumably due to a retry",
							backref, desc.ID,
						)
						found = true
						break
...
func runSchemaChangesInTxn(
	...
	for _, c := range constraintsToValidate {
		...
		case sqlbase.ConstraintToUpdate_FOREIGN_KEY:
			if err := validateFkInTxn(ctx, tc.leaseMgr, evalCtx, tableDesc, txn, &c.Name); err != nil {
				return err
			}
		}
	}
...
func validateFkInTxn(
	...
	indexDesc, err := tableDesc.FindIndexByID(id)
	if err != nil {
		panic(err)
	}
	...
	return validateForeignKey(ctx, tableDesc.TableDesc(), indexDesc, ie, txn)
}
